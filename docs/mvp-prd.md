...
- Composition 保留每个子素材的颜色槽位；更换 Colorway 时，各子素材仍可独立映射。
- Composition 计算变换后的联合 Bounds 与碰撞半径，Arrange 将其作为一个整体参与间距判断。
- Input 中移除正在被启用的 Compose 使用的素材时不得静默换成其他素材；系统阻止操作并给出恢复路径。
...
- Arrange 只输出 `Layout Scene`：Motif 引用、位置、缩放、旋转、碰撞半径和稳定颜色索引；不写入最终颜色。
- Artboard / Tile 属于全局 Surface Context，并在 Arrange 计算前生效；Repeat 的环绕碰撞不是 Output 的末端装饰。
...
- Organic 提供 Classic 与 Dense：Classic 接受第一个有效候选；Dense 对多个候选评分、优先放置大图形，并用最小图形执行补位。
- 使用空间哈希查询附近实例；Repeat Surface 下同时查询对侧邻居。
- 达到画布容量时不允许强制重叠，保留已成功实例并显示“实际放置 / 目标数量”和恢复建议。
- 实时预览。
- Bypass。
- Shuffle Seed。
...
| Fill Strategy | Classic / Dense；Dense 使用多候选评分和小图形补位 |
| Seed | 固定随机结果 |
...
| Motif Weight | Weighted 模式及 Organic 中控制各 Motif 出现频率，范围 0.25×–3× |
...
- Colorway 消费 `Layout Scene` 并输出 `Styled Scene`，只映射背景和颜色，不改变实例位置、缩放、旋转或 Motif 分配。
...
用户确定输出尺寸并导出可继续使用的 SVG；是否使用无缝 Tile 已由全局 Surface Context 决定。

**功能**

- 显示当前全局 Surface Context，但 Output 不再拥有独立的 Artboard / Repeat 模式。
- 提供 Square / Landscape / Portrait 快速尺寸和精确 Width / Height。
- 设置 Artboard 或 Tile Width / Height。
- 单位：px / mm。
- Artboard Surface：有限画板预览、背景和导出，不生成环绕副本。
- Tile Surface 的 Repeat Structure：
  - Square。
  - Brick。
  - Half Drop。
...
- 显示中心 Tile。
- 切换背景色。
- Canvas 维护独立的 `viewedNode`，Inspector 维护 `selectedNode`；“View final output”只改变查看目标，不会把 Inspector 强制切到 Output。
- 查看 Input 或 Compose 时，Canvas 直接展示该节点输出的 Motif Set，而不是最终图样。
- 查看 Arrange 或 Colorway 时，Canvas 展示单 Scene；只有查看 Output 且 Surface 为 Tile 时展示多 Tile 最终预览。
...
- 实例默认不可逐个编辑。
- 选中 Arrange 后显示布局边界、目标数量、实际放置数量和容量状态。
- Shuffle Seed 只修改 Seed，不修改其他参数。
- MVP 不提供 Bake 后逐实例编辑。
- Organic 任何策略都不得为了满足目标数量强制写入碰撞实例。

## 13. Network 交互

### 13.1 默认能力

- 点击节点时 Inspector 与 Canvas 默认同步；用户也可单独查看最终输出，节点卡分别标识正在编辑和正在查看的状态。
- 拖动节点位置。
- 缩放、平移和 Fit Graph。
- 显示类型化端口和连接。
...
- Inspector 内容只跟随当前选中节点，不因“View final output”改变。
