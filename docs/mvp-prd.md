...
新项目不展示空白节点画布，系统创建默认 Recipe：

```text
Input → Arrange → Colorway → Output
```

当用户需要把多个素材拼成固定图形时插入 Compose：
...
- 支持两种输出方式：
  ...
  - Append to Set：输出组合后的 Motif，并透传未参与组合的素材。
- 默认使用 Append to Set，避免组合后意外丢失其他素材。
